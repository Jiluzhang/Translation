...
from multiprocessing import Pool
def cnt(tf):
    return sum((tf_attn[tf].flatten())>((0.000001)*tf_exp_cnt[tf]))
...
# import random
# cr = df['tf'][random.sample(range(df.shape[0]), 500)].values
...
p.save(filename='CR_Non_CR_0.000001_box_2.pdf', dpi=100, height=4, width=4)
...
p.save(filename='CR_Non_CR_0.001_box_2.pdf', dpi=100, height=4, width=4)
...
p.save(filename='CR_Non_CR_0.01_box_2.pdf', dpi=100, height=4, width=4)
...
cut -f 7 /fse/home/dongxin/Projects/0Finished/SCRIPT/motif/human/human_cisBP/TF_Information.txt | sed '1d' | sort | uniq
...
df = pd.read_csv('fibro_tf_peak_0_no_norm.txt', header=None, sep='\t')
df.columns = ['tf', 'cnt']
df_top20 = df.sort_values('cnt', ascending=False)[:20]
tf_top20 = list(df_top20['tf'])
tf_top20.reverse()
