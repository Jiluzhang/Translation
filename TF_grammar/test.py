...
def main():
    ...
    with open(exp_wig_filename, "w") as exp_f, open(norm_wig_filename, "w") as norm_f:
        for chrom, start, end in zip(grs.Chromosome, grs.Start, grs.End):
            ...
            #Luz signal_raw[np.isnan(signal_raw)] = 0
            #Luz signal_bias[np.isnan(signal_bias)] = 0

            # get expected values
            signal_exp = np.zeros(shape=(end - start))
            for i in tqdm(range(half_window, len(signal_raw) - half_window), ncols=80): #Luz for i in range(half_window, len(signal_raw) - half_window):
                total_signal = np.nansum(signal_raw[i - half_window : i + half_window]) #Luz total_signal = np.sum(signal_raw[i - half_window : i + half_window])
                total_bias = np.nansum(signal_bias[i - half_window : i + half_window]) #Luz total_bias = np.sum(signal_bias[i - half_window : i + half_window])
            ...
            signal_norm = np.subtract( #Luz signal_norm = np.divide(
